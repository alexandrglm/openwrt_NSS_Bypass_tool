...
nft_generate_script() {
    ...
    echo ''                                                                            >> "$FW_SCRIPT"
    echo "    nft add rule inet fw4 \${NFT_CHAIN_POST} meta mark and \${NSS_MARK} != 0 ct mark set meta mark and \${NSS_MARK} comment '$c1'" >> "$FW_SCRIPT"
    echo "    nft add rule inet fw4 \${NFT_CHAIN_PRE}  ct mark and \${NSS_MARK} != 0 meta mark set ct mark comment '$c2'" >> "$FW_SCRIPT"
    echo '}'                                                                           >> "$FW_SCRIPT"
    echo ''                                                                            >> "$FW_SCRIPT"
